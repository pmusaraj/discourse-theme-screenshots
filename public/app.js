...
function render() {
  ...
  const themeId = location.hash.startsWith("#/theme/")
    ? decodeURIComponent(location.hash.slice(8))
    : "";

  const theme =
    themeId && themeId !== location.hash
      ? (manifest?.themes ?? []).find((item) => item.id === themeId)
      : null;
...
function renderIndex() {
  ...
  summary.textContent = manifest
    ? themes.length +
      " themes · subset " +
      manifest.screenshot?.subset +
      " · generated " +
  ...
  app.innerHTML = themes
    .map((theme) => {
      ...
      return (
        ...
        previewImage(preview, theme.name, cacheBust) +
        "</div>" +
        "</a>" +
        "</article>"
      );
...
function renderTheme(theme) {
  ...
  const warnings = (theme.warnings ?? []).length
    ? '<ul class="warnings">' +
      theme.warnings
  ...
  lightboxItems = ["desktop", "mobile"].flatMap((device) =>
    grouped[device].map((src) => ({
      device,
      src,
  ...
  const sections = ["desktop", "mobile"]
    .map((device) => renderCarouselSection(device, grouped[device], cacheBust))
    .join("");

  app.innerHTML =
    '<nav class="back"><a href="#/">← all themes</a></nav>' +
    '<section class="detail-head"><div><h2>' +
    escapeHtml(theme.name) +
...
function sortScreenshotFiles(files) {
  ...
  const prioritized = [
    ...
    const index = remaining.findIndex((src) =>
      screenshotBaseName(src).endsWith(suffix),
    );
...
function renderCarouselSection(device, files, cacheBust) {
  ...
  const shots = files
    .map((src) => {
      ...
      return (
        '<li class="shot-row"><figure><a class="shot-link" href="' +
        escapeHtml(url) +
        '" data-lightbox-index="' +
        lightboxIndex +
        '" aria-label="Open ' +
  ...
  return (
    '<section class="device-section device-section--' +
    escapeHtml(device) +
    '">' +
    '<div class="device-head"><h3>' +
    escapeHtml(device) +
    "</h3><span>" +
    files.length +
    " screenshots</span></div>" +
    '<ol class="screenshot-carousel">' +
    shots +
    "</ol>" +
    "</section>"
  );
...
function previewImage(src, themeName, cacheBust) {
  ...
  return (
    ...
    ' topic list" loading="lazy"></figure>'
  );
}
...
function ensureLightbox() {
  ...
  lightbox.innerHTML =
    '<button class="lightbox-close" type="button" aria-label="Close full-size screenshot">×</button>' +
...
loadManifest()
  ...
  .catch((error) => {
    ...
    app.textContent =
      "Run bin/screenshot-themes to generate screenshots and manifest data.";
  });
