...
class ManifestTest < Minitest::Test
  def test_manifest_includes_raw_files_and_representative_screenshots
    ...
    config = ThemeScreenshots::Config.new(path: "config.yml", discourse: { "repo_path" => "/tmp/discourse" }, screenshot: { "subset" => nil, "devices" => "desktop" }, themes: [theme])
    ...
    assert_nil manifest[:screenshot][:subset]
    assert_equal "desktop", manifest[:screenshot][:devices]
